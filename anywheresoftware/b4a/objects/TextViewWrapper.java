...
public class TextViewWrapper<T extends TextView> extends ViewWrapper<T> implements DesignerTextSizeMethod {
    ...
    public static View build(Object prev, Map<String, Object> props, boolean designer) throws Exception {
        ...
        v.setGravity(((Integer) Gravity.class.getField((String) props.get("hAlignment")).get(null)).intValue() | ((Integer) Gravity.class.getField((String) props.get("vAlignment")).get(null)).intValue());
        style = ((Integer) props.get("textColor")).intValue();
        if (style != ViewWrapper.defaultColor) {
            v.setTextColor(style);
        }
        if (designer && style == ViewWrapper.defaultColor) {
